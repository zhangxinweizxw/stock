...
import (
	...
	"stock/share/garyburd/redigo/internal/redistest"
)
...
func TestDialURL(t *testing.T) {
	...
	c, err := DialURL("redis://localhost", DialNetDial(checkPort))
	if err != nil {
	...
	c, err = DialURL("redis://:6379", DialNetDial(checkHost))
	if err != nil {
		t.Error("dial error:", err)
	}
	c.Close()
	...
	c1, err := DialURL("redis://:6379/8")
	defer c1.Close()
	if err != nil {
		t.Error("Dial error:", err)
	}
	dbSize, _ := Int(c1.Do("DBSIZE"))
	if dbSize > 0 {
		t.Fatal("DB 8 has existing keys; aborting test to avoid overwriting data")
	}
	c1.Do("SET", "var", "val")

	c2, err := Dial("tcp", ":6379")
	defer c2.Close()
	if err != nil {
		t.Error("dial error:", err)
	}
